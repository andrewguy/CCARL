...
from ccarl.validate_cfg_structures import validate_cfg_structures, ArrayMismatchError
from ccarl.glycan_graph_methods import digraph_to_glycan_string
from ccarl.ccarl import CCARLClassifier, _log_rfu_values, _calculate_binders
from ccarl.plotting.microarray import plot_log_rfu_histogram
from ccarl.plotting.utils import remove_top_right_borders
from ccarl.utils import _setup_logging
...
@click.option('-v', '--verbose', is_flag=True,
              help='Verbose output.')
def identify_motifs(input, output_prefix, support_positive, support_all, format, cross_validation, verbose):
    ...
    for i, feature in enumerate(cf.subtree_features):
        feature_str = digraph_to_glycan_string(feature)
        print(f"Feature {i}: {feature_str}")
    # Write out performance
    # AUC, accuracy, MCC etc
    # Optional: Predict binding for unknown glycans.
